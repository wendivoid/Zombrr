use bevy::prelude::*;
use zombrr_core::{ ZombrrPackages, ArenaOptions, ZombrrState, MenuState };
...
fn render_change_map(ui: &mut egui::Ui, _: &mut Settings, world: &mut World) {
    let (selected, original) = {
        ...
        (
            ...
            (original_map.0.to_string(), original_map.1.to_string(), original_map.2.to_string())
        )
    };
    let mut events = world.get_resource_mut::<bevy::app::Events<ChangeMap>>().unwrap();
    if selected.0 != original.0 ||
       selected.1 != original.1 ||
       selected.2 != original.2 {
