...
pub fn settings() -> Setting {
    Setting::new_labeled("rapier-hilt", "Rapier Hilt").set_value(SettingValue::Group(vec![
...
pub fn show_colliders_changed(
    ...
    if let Some(setting) = settings.get_key(&["rapier-hilt", "show-colliders"]) {
        if let Some(value) = setting.value.as_bool() {
            if *last_value != value {
                *last_value = value;
                events.send(bevy_hilt::HiltToggleVisibility(
                    bevy_hilt::HiltEntities::All,
                ));
            }
        } else {
...
pub fn show_render_pass_changed(
    ...
    if let Some(setting) = settings.get_key(&["rapier-hilt", "render-pass"]) {
        if let Some(value) = setting.value.as_bool() {
            if *last_value != value {
                *last_value = value;
                events.send(bevy_hilt::HiltToggleRenderPass(
                    bevy_hilt::HiltEntities::All,
                ));
            }
        } else {
