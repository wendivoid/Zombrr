...
fn perform_spawn_enemy(world: &mut World) {
    let spawn_event = {
        ...
        let pos_x = settings.get_key(&["enemy", "default_pos_x"]).unwrap().value.as_float().unwrap();
        let pos_y = settings.get_key(&["enemy", "default_pos_y"]).unwrap().value.as_float().unwrap();
        let pos_z = settings.get_key(&["enemy", "default_pos_z"]).unwrap().value.as_float().unwrap();
        let speed = settings.get_key(&["enemy", "speed"]).unwrap().value.as_float().unwrap();
        SpawnEnemy {
...
fn render_spawn_enemy(ui: &mut Ui, settings: &mut Settings, _: &mut World) {
    ui.horizontal(|ui| {
        ...
        ui.columns(3, |ui| {
            {
                let pos_x = settings.get_key_mut(&["enemy", "default_pos_x"]).unwrap().value.as_float_mut().unwrap();
                ui[0].add(egui::DragValue::new(pos_x));
            }
            {
                let pos_y = settings.get_key_mut(&["enemy", "default_pos_y"]).unwrap().value.as_float_mut().unwrap();
                ui[1].add(egui::DragValue::new(pos_y));
            }
            {
                let pos_z = settings.get_key_mut(&["enemy", "default_pos_z"]).unwrap().value.as_float_mut().unwrap();
    ...
    ui.horizontal(|ui| {
        ui.columns(2, |ui| {
            let speed = settings.get_key_mut(&["enemy", "speed"]).unwrap().value.as_float_mut().unwrap();
